...
void init_scheduler(){
	...
	if(scheduler == NULL) exit(-1);

	if (pthread_create (&scheduler->long_term, NULL, schedule_long_term, NULL) != 0){
		print_pthread_create_error("schedule_long_term");
		destroy_scheduler(scheduler);
		exit(-1);
	}

	if (pthread_create (&scheduler->short_term, NULL, schedule_short_term, NULL) != 0){
...
void new_program(int client_fd){
	...
	t_program *programa = create_program(client_fd);
...
void *schedule_short_term(void *arg){
	...
	log_debug(logger, "[OK] Planificador de Corto Plazo");

	while(!endsuse){

	}

	log_debug(logger, "Finalizando Planificador de Corto Plazo");
...
void program_remove_tcb_from_state(int pid, pid_t tid, int state){
	...
	t_program *program = get_program(pid);
...
void program_add_tcb_to_state(t_tcb *tcb, int state){
	...
	t_program *program = get_program(tcb->pid);
...
t_program *create_program(int client_socket){
	...
	programa->pid = ++program_id_counter;
	programa->socket = client_socket;
	programa->ready = list_create();
	programa->exec = NULL;

	log_debug(logger,"Nuevo Programa - id = %d", programa->pid);
	return programa;
}

t_scheduler *create_scheduler(){

	t_scheduler *scheduler = malloc(sizeof(t_scheduler));
	if (scheduler == NULL){
	   print_malloc_error("Scheduler");
	   return NULL;
	}
